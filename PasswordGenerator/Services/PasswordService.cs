...
public class PasswordService : IPasswordService
{
    ...
    public GeneratedPassword GeneratePassword(PasswordGenerationRequest request)
    {
        if (string.IsNullOrWhiteSpace(request.MasterPassword))
            throw new ArgumentException("Master password cannot be empty", nameof(request.MasterPassword));
        
        if (string.IsNullOrWhiteSpace(request.Seed))
            throw new ArgumentException("Seed cannot be empty", nameof(request.Seed));
