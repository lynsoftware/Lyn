...
namespace Lyn.Backend.Apps.PasswordGenerator.Services;
    ...
    public async Task<Result<PasswordGenerationResponse>> GeneratePasswordAsync(PasswordGenerationRequest request)
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                var statisticsRepository = scope.ServiceProvider.GetRequiredService<IPasswordGeneratorStatisticsRepository>();
